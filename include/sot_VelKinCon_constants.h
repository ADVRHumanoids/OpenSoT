...
static const paramHelp::ParamBilatBounds<int> SOT_POSTURAL_WEIGHT_STRATEGY_BOUNDS(ParamBilatBounds<int>(0,3));
...
new ParamProxyBasic<double>("max_joint_velocity",                  PARAM_ID_MAX_JOINT_VELOCITY,                  1,        &SOT_MAX_JOINT_VELOCITY_BOUNDS,        PARAM_IN_OUT,     &SOT_DEFAULT_MAX_JOINT_VELOCITY,                         "Maximum velocity [rad/sec] for the joints"),
...
new ParamProxyBasic<int>("last_stack_type",                        PARAM_ID_LAST_STACK_TYPE,                     1,        &SOT_LAST_STACK_TYPE_BOUNDS,           PARAM_CONFIG,     &SOT_DEFAULT_LAST_STACK_TYPE,                            "type of last stack. It is an enum, with values 0: postural (A=I, b=(q-q_ref)) 1: postural and gravity gradient (A=[I;grad_g(q)], b=[q-q_ref;0]) 2: (gravity torque) minimum effort (A=I, b=grad_g(q)^T) 3: postural and minimum effort (wrt gravity) (A=[I;I], b =[q-q_ref;grad_g(q)^T])"),
new ParamProxyBasic<int>("postural_weight_strategy",               PARAM_ID_POSTURAL_WEIGHT_STRATEGY,            1,        &SOT_POSTURAL_WEIGHT_STRATEGY_BOUNDS,  PARAM_CONFIG,     &SOT_DEFAULT_POSTURAL_WEIGHT_STRATEGY,                   "postural stack weight strategy. It is an enum, with values 0: postural uses Identity weight matrix 1: postural uses weight which lets distal joints move further away. The weight is normalized w.r.t. joint limits 2: postural uses diag(grad_g(q)) 3: postural uses joint-space Inertia matrix"),
