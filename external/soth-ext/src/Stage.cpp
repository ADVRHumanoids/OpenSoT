...
#define SOTH_STAGE_COMMON_CONSTRUCTOR                                       \
  W_(nr, nr), ML_(nr, nc), e_(nr), lambda_(nr)                              \
      ...
      Ir(), Irn(), Iw(), Im(), Il()                                         \
                                                                            \
                                   ,                                        \
      M(ML_, &Irn, &Im), L(ML_, &Ir, &Il), W(W_, &Iw, &Irn)                 \
                                                                            \
                                               ,                            \
      Wr(W_, &Iw, &Ir), Mr(ML_, &Ir, &Im), e(e_, &Iw), lambda(lambda_, &Iw) \
                                                                            \
                                                           ,                \
      sizeM(0), sizeL(0)                                                    \
                                                                            \
                    ,                                                       \
      activeSet(nr, Iw), freeML(nr)                                         \
                                                                            \
                             ,                                              \
      Ld_(nr, nr), Ldwork_(nr, nr), edwork_(nr), Ld(Ld_, false, false),     \
      Ldwork(Ldwork_, &Ld.getRowIndices(), &Ld.getColIndices()),            \
      edwork(edwork_, &Ld.getRowIndices()), Wd(),                           \
      dampingFactor(DAMPING_FACTOR)                                         \
                                                                            \
          ,                                                                 \
      isReset(false), isInit(false), isOptimumCpt(false),                   \
      isLagrangeCpt(false), isDampCpt(false), isFreezed(false)
...
Stage::Stage(const MatrixXd& J, const VectorBound& bounds, BaseY& Y)
    : BasicStage(J, bounds, Y), SOTH_STAGE_COMMON_CONSTRUCTOR {
  SOTH_STAGE_COMMON_INIT;
}

Stage::Stage(const Index IN_nr, const Index IN_nc, const double* IN_Jdata,
             const Bound* IN_bdata, const BaseY& IN_Y)
...
Stage::Stage(const Index IN_nr, const Index IN_nc, const double* IN_Jdata,
             const BaseY& IN_Y)
    : BasicStage(IN_nr, IN_nc, IN_Jdata, IN_Y), SOTH_STAGE_COMMON_CONSTRUCTOR {
  SOTH_STAGE_COMMON_INIT;
}
...
void Stage::setInitialActiveSet(void) {
  activeSet.reset();
...
void Stage::setInitialActiveSet(const cstref_vector_t& initialGuess,
                                bool checkTwin) {
  sotDEBUGIN(5);
  activeSet.reset();
...
cstref_vector_t Stage::getOptimalActiveSet(bool) {
  cstref_vector_t res(sizeA());
  int loop = 0;
  for (Index i = 0; i < sizeA(); ++i) {
    if (activeSet.whichBoundInv(i) != Bound::BOUND_TWIN)
      res[loop++] = ConstraintRef(activeSet.whichConstraint(i),
                                  activeSet.whichBoundInv(i));
  }
  res.resize(loop);
  return res;
}
...
void Stage::computeInitialJY(void) {
  ...
  ML = SubMatrix<MatrixXdRef, RowPermutation>(const_cast<MatrixXdRef&>(J),
                                              &activeCst);
  sotDEBUG(15) << "Ja = " << (MATLAB)ML << std::endl;

  freeML.resetTop(sizeA());

  const SubMatrix<VectorBoundRef, RowPermutation> ba(
      const_cast<VectorBoundRef&>(bounds), &activeCst);
...
bool Stage::testUnactiveTwins(void) {
#ifndef NDEBUG
  ...
  for (Index cst = 0; cst < nr; ++cst) {
    if ((bounds[cst].getType() == Bound::BOUND_TWIN) &&
      ...
      std::cout << bounds[cst].getType() << " " << activeSet.isActive(cst)
                << std::endl;
      return false;
    }
  }
 ...
 * stored in the stage. */
void Stage::computeInitialCOD(BaseY& Yinit) {
  /*
   * ML=J(initIr,:)*Y;
  ...
  /* M=submatrix(ML,1:previousRank); L=submatrix(ML,previousRank+1:end); */
  M.setColRange(0, previousRank);
  M.setRowRange(0, sizeA());
  sizeM = previousRank;
  L.setColRange(previousRank, nc);
  L.setRowRange(0, sizeA());
  sizeL = sizeA();

  sotDEBUG(15) << "MY = " << (MATLAB)M << std::endl;
  sotDEBUG(15) << "LY = " << (MATLAB)L << std::endl;
  sotDEBUG(5) << "e = " << (MATLAB)e << std::endl;
  sotDEBUG(25) << "sizesAML = [" << sizeA() << ", " << sizeM << ", " << sizeL
               << "]." << endl;

  if ((L.cols() == 0) || (L.rows() == 0)) {
    sotDEBUG(5) << "col size of L is null, skip the end of initialization"
                << endl;
    L.setRowIndices(Matrix<Index, Dynamic, 1>());
    L.setColIndices(Matrix<Index, Dynamic, 1>());
    sizeL = 0;
    return;
  }
  ...
  Block<MatrixXd> Lblock =
      ML_.block(0, previousRank, sizeA(), nc - previousRank);
  Transpose<Block<MatrixXd> > Lt = Lblock.transpose();
  typedef Eigen::DestructiveColPivQR<Transpose<Block<MatrixXd> >,
                                     Block<MatrixXd> >
      DestructiveColPivQR;
  ...
  */
  ...
  sotDEBUG(45) << "mQ = " << (MATLAB)Yinit.getHouseholderEssential()
               << std::endl;
...
void Stage::conditionalWinit(bool id) {
  if (id) {
    isWIdenty = true;
  } else if (isWIdenty) {
    isWIdenty = false;
 ...
 */
void Stage::nullifyLineDeficient(const Index row, const Index in_r) {
  assert(!isWIdenty);
  /*
   * Jd = L.row(r);
...
void Stage::removeARowFromL(Index row) {
  assert(!isWIdenty);
  L.removeRow(row);
 ...
 */
void Stage::freezeSlacks(const bool& slacks) {
  ...
  EI_FOREACH(i, lambda) {
    if (std::abs(lambda[i]) > EPSILON) {
      ...
      if (slacks) {
        e[i] += lambda[i];
        sotDEBUG(5) << "Freeze cst " << name << ":" << cstref << " to " << e[i]
                    << endl;
      }
    }
  }
...
bool Stage::downdate(const Index position, GivensSequence& Ydown) {
  notifior(name,
  ...
  if (rankDef) {  // Full-rank line removed and no rank promotion: resorbe
                  // Hessenberg and propagate.
...
bool Stage::propagateDowndate(GivensSequence& Ydown,
  ...
  L.popColBack();
  sotDEBUG(5) << "L = " << (MATLAB)L << std::endl;
  SubCol<MatrixXd>(ML_, Irn, (int)sizeM)
      .head(sizeN())
      .setZero();  // PSEUDOZEROS
 ...
 */
MatrixXd::Index Stage::nullifyACrossFromW(const Index row) {
  sotDEBUG(5) << "W0 = " << MATLAB(W, isWIdenty) << std::endl;
  sotDEBUG(5) << "M0 = " << (MATLAB)M << std::endl;
  sotDEBUG(5) << "L0 = " << (MATLAB)L << std::endl;
...
void Stage::removeACrossFromW(const Index& row, const Index& col) {
...
MatrixXd::Index Stage::update(const ConstraintRef& cst, GivensSequence& Yup) {
  // usleep(1000*0.01);
  ...
  JupY = sign * J.row(cst.row);
  Y.applyThisOnTheLeft(JupY);
  ...
  for (Index i = nc - 1; i >= int(sizeM); --i) {
    ...
    if (norm2 > EPSILON * EPSILON) {
      sotDEBUG(45) << "Oversize value is x" << i << " = " << JupY(i)
                   << std::endl;
      rankJ = (Index)i + 1;
      break;
    }
  ...
  if (rankJ > sizeM + sizeL) { /* Rank increase: remove the tail of JuY. */
  ...
  } else { /* No rank increase: regularize. */
    conditionalWinit(false);
    if (rankJ > sizeM) {
      addARow(wcolup);
      nullifyLineDeficient(sizeL - 1, rankJ - sizeM);
    } else { /* L-part of the new line null, no need to regularize. */
      addARow(wcolup, true);
    }
  }
...
void Stage::propagateUpdate(GivensSequence& Ydown, Index decreasePreviousRank) {
  assert(isInit);
  isLagrangeCpt = false;
  isOptimumCpt = false;
  isDampCpt = false;
...
void Stage::addARow(const Index& wcolup, bool deficient) {
...
void Stage::computeSolution(VectorXd& Ytu) const {
  ...
  if (!isWIdenty) {
    sotDEBUG(45) << "Wr = " << (MATLAB)Wr << endl;
  } else {
    sotDEBUG(45) << "W = " << MATLAB(W, isWIdenty) << endl;
  }
...
void Stage::applyDamping(MatrixBase<VectorDerived>& x) const {
...
void Stage::applyDamping(MatrixBase<VD1>& x, MatrixBase<VD2>& y) const {
 ...
 */
template <typename VectorDerived>
void Stage::applyDampingTranspose(MatrixBase<VectorDerived>& x) const {
  typedef MatrixBase<VectorDerived> VectorBase;
  EIGEN_STATIC_ASSERT_VECTOR_ONLY(VectorBase);
  assert(isDampCpt);
  assert(x.size() == L.cols() && x.size() == L.rows());
...
template <typename VD1, typename VD2>
void Stage::applyDampingTranspose(MatrixBase<VD1>& x,
...
void Stage::dampBoundValue(const ConstraintRef& cst, const double& value) {
 ...
 */
template <typename D>
void Stage::computeErrorFromJu(const VectorXd& MLYtu,
                               MatrixBase<D>& err) const {
 ...
 */
void Stage::computeMLYtu(const VectorXd& Ytu, VectorXd& MLYtu) const {
  assert(false && "DEPRECATED");

  assert(Ytu.size() == int(nc));
  MLYtu.noalias() = M * Ytu.head(sizeM);
  MLYtu.tail(sizeL).noalias() += getLtri() * Ytu.segment(sizeM, sizeL);
 ...
 */
template <typename D>
void Stage::computeError(const VectorXd& Ytu, MatrixBase<D>& err) const {
  assert(false && "DEPRECATED");
 ...
 */
void Stage::computeRho(const VectorXd& Ytu, VectorXd& Ytrho, bool inLambda) {
  assert(isInit);
  assert(Ytu.size() == int(nc));

  if (inLambda) { /* Lagrang of the last stage. */
    if (sizeN() == 0) {
  ...
  if (isDampCpt) {
    ...
    if (sizeM > 0) {
      for (int i = 0; i < (int)sizeM; ++i) {
        if (!M.col(i).isZero(EPSILON)) Ytrho[i] -= eta * eta * zbar[i];
      }
    }
 ...
 */
template <typename D>
void Stage::computeLagrangeMultipliers(VectorXd& rho, MatrixBase<D>& l) const {
  EIGEN_STATIC_ASSERT_VECTOR_ONLY(MatrixBase<D>);
  assert(isInit);
...
void Stage::computeLagrangeMultipliers(VectorXd& rho) {
  assert(isInit);
...
bool Stage::checkBound(const VectorXd& u0, const VectorXd& u1,
  ...
  for (Index i = 0; i < nr; ++i) {
    ...
    sotDEBUG(5) << "Ju0=" << val0 << "  --  Ju1=" << val1
                << " -- dval=" << val1 - val0 << endl;

    Bound::bound_t btype1;
    if (isDampCpt)
      btype1 = b.check(val1, activeSet.getBoundDamping(i), EPSILON);
    else
      btype1 = b.check(val1, EPSILON);
...
bool Stage::checkBound(const VectorXd& u0, const VectorXd& u1,
  ...
  if ((tauptr == NULL) && (cstptr == NULL)) {
    double tau = 1;
    ConstraintRef cst;
    return checkBound(u0, u1, cst, tau);
  } else if ((tauptr != NULL) && (cstptr == NULL)) {
    ConstraintRef cst;
    return checkBound(u0, u1, cst, *tauptr);
  } else if ((tauptr == NULL) && (cstptr != NULL)) {
    double tau = 1;
    return checkBound(u0, u1, *cstptr, tau);
...
bool Stage::  // TODO: Ytu could be passed instead of u. TODO! u is not usefull
              // any more.
  ...
  EI_FOREACH(i, lambda) {
    ...
    switch (btype)  // TODO: the code is the same for +/-, factorize or change
                    // the sign of J.
    {
      case Bound::BOUND_TWIN:
        break;  // Nothing to do.
      case Bound::BOUND_SUP:
        sotDEBUG(5) << name << ": row" << i << ", cst" << which((Index)i)
                    << ": l=" << lambda(i, 0) << endl;
        if (-lambda[i] > lmax) {
...
Stage::RowML Stage::rowMrL0(const Index r) { return ML_.row(Ir(r)); }
...
Stage::Index Stage::rowSize(const Index r) {
  if (r < sizeN())
    ...
    return std::min((Index)(sizeM + r - sizeN() + 1), nc);
}
...
void Stage::recompose(MatrixXd& WMLY) const {
  //    sotDEBUGPRIOR(+40);
  if (sizeA() == 0) {
  ...
  WMLY.resize(sizeA(), nc);
  WMLY.setZero();
  if (isWIdenty)
    WMLY.block(0, 0, sizeA(), sizeM) = M;
  else
  ...
  if (sizeL != 0) {
    if (isWIdenty)
      WMLY.block(0, sizeM, sizeA(), sizeL) = L;
    else
      WMLY.block(0, sizeM, sizeA(), sizeL) = Wr * L;
  }
...
bool Stage::testRecomposition(void) const {
  /* When damp is used, recomposition is wrong after freezing. */
  if (isFreezed) return true;
  MatrixXd Jrec;
  recompose(Jrec);
  MatrixXd Ja_;
  SubMatrix<MatrixXd, RowPermutation> Ja = Jactive(Ja_);
  ...
  if (sizeA()) {
    ...
    res = ((Jrec.cols() == Ja.cols()) && (Jrec.rows() == Jrec.rows()));
  sotDEBUG(5) << "% J: Recomposition  " << ((res) ? "OK" : "wrong")
              << " (||.||=" << norm << ")." << std::endl;

  VectorXd ea_;
  bool vres =
      true;  // DEBUG: the test is wrong when the stage has been freezed.
  ...
  // sotDEBUG(5) <<"% e: Recomposition  " << ((vres)?"OK.":"wrong.") <<
  // std::endl;
...
bool Stage::testSolution(const VectorXd& /*solution*/) const {
...
Stage::Index Stage::where(Index cst) const { return activeSet.map(cst); }
ConstraintRef Stage::which(Index row) const {
  assert(row < sizeA());
  ConstraintRef res;
...
bool Stage::isActive(Index cst) const {
  assert(cst < nr);
...
SubMatrix<MatrixXd, RowPermutation> Stage::Jactive(MatrixXd& J_) const {
  J_.resize(nr, nc);
  J_.setConstant(-1.11111);
  for (Index cst = 0; cst < nr; ++cst) {
...
SubVectorXd Stage::eactive(VectorXd& e_) const {
  e_.resize(nr);
  e_.setConstant(-1.11111);

  for (Index cst = 0; cst < nr; ++cst) {
    if (activeSet.isActive(cst)) {
      e_(activeSet.where(cst)) =
          activeSet.sign(cst) * bounds[cst].getBound(activeSet.whichBound(cst));
    }
  }
...
void Stage::show(std::ostream& os, Index stageRef, bool check) const {
  sotDEBUGPRIOR(+20);
  ...
  sotDEBUG(5) << "erec{" << stageRef << "} = " << (MATLAB)eactive()
              << std::endl;
  ...
  if (check) {
    ...
    if (Jrec.rows() > 0) {
      ...
      if ((e - eactive()).norm() <= 1e-6)
        ...
        os << "% Recomposition e not OK. " << std::endl;
    } else {
      sotDEBUG(5) << "Jrec=" << (MATLAB)Jrec << endl;
      os << "% Recomposition OK. " << std::endl;
      sotDEBUG(5) << "% Recomposition e OK. " << std::endl;
...
void Stage::showActiveSet(std::ostream& os) const {
