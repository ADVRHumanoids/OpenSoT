...
ActiveSet::ActiveSet(unsigned int nr)
    : cstMap(nr),
      cstMapInv(nr),
      freerow(nr),
      freezed(nr),
 ...
 * with bound type. */
void ActiveSet::active(unsigned int ref, Bound::bound_t type,
                       unsigned int row) {
  assert(ref < size());
...
unsigned int ActiveSet::activeRow(unsigned int ref, Bound::bound_t type) {
 ...
 */
void ActiveSet::unactiveRow(unsigned int row) {
  assert(row < size());
...
void ActiveSet::freeze(unsigned int ref) {
  assert(ref < size());
  assert(cstMap[ref].type != Bound::BOUND_NONE);
...
bool ActiveSet::isRowFree(unsigned int row) { return freerow[row]; }
unsigned int ActiveSet::getAFreeRow(void) {
...
void ActiveSet::freeARow(unsigned int row) {
  assert(!freerow[row]);
  freerow[row] = true;
}

void ActiveSet::dampBoundValue(const ConstraintRef& cst, const double& value) {
...
std::pair<double, double> ActiveSet::getBoundDamping(const unsigned int& cst) {
...
unsigned int ActiveSet::mapInv(unsigned int row) const {
  assert(row < size());
  assert((cstMapInv[row] < size()) && "The requested row is not active");
  return cstMapInv[row];
}
unsigned int ActiveSet::map(unsigned int ref) const {
  assert(isActive(ref));
  return cstMap[ref].row;
}
Bound::bound_t ActiveSet::whichBound(unsigned int ref, bool checkActive) const {
  assert(isActive(ref));
...
bool ActiveSet::isActive(unsigned int ref) const {
  assert(ref < size());
  return (cstMap[ref].type != Bound::BOUND_NONE);
}
bool ActiveSet::wasActive(unsigned int ref, const Bound::bound_t type) const {
  assert(ref < size());
...
void ActiveSet::disp(std::ostream& os, bool classic) const {
  if (classic) {
    os << " [ ";
...
/*   DPC */ {
  ...
  /*   DPC */ for (unsigned int i = 0; i < nba; ++i)
    Pt(P(i)) = i;
  /*   DPC */ for (unsigned int i = 0; i < size(); ++i)
    /* DPC */ if (isActive(i))
      cstMap[i].row = Pt(cstMap[i].row);
    /*   DPC */}
