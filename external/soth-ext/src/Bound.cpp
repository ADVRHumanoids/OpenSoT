...
namespace soth {
Bound::Bound(void) : type(BOUND_NONE), valInf(0), valSup(0), valTwin(valInf) {}
...
const double& Bound::getBound(bound_t inType) const {
  switch (inType) {
    case BOUND_INF:
...
Bound::bound_t Bound::check(const double& val, const double& EPSILON) const {
  switch (type) {
    case BOUND_INF:
...
/* Return the bound that is violated, NONE if bound are OK.
 * In case of twin-bounds, no check is performed, NONE is always returned. */
Bound::bound_t Bound::check(const double& val, std::pair<double, double> damp,
...
/* Return the bound that is violated, NONE if bound are OK.
 * In case of twin-bounds, no check is performed, NONE is always returned. */
Bound::bound_t Bound::checkSaturation(const double& val,
                                      const double& EPSILON) const {
  switch (type) {
    case BOUND_INF:
