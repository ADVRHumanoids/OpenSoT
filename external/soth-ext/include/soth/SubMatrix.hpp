...
struct evaluator<SubMatrix<MatrixType, PermutationType, IsSub> >
  ...
  inline CoeffReturnType coeff(Index row) const { return m_argImpl.coeff(row); }
...
struct ei_submatrix_index_helper<MatrixType, RowPermutation, rowsAtCompileTime,
                                 colsAtCompileTime> {
...
template <typename MatrixType, int rowsAtCompileTime, int colsAtCompileTime>
struct ei_submatrix_index_helper<MatrixType, ColPermutation, rowsAtCompileTime,
                                 colsAtCompileTime> {
  typedef typename MatrixType::Index Index;
...
struct ei_choose_assert_selection<RowPermutation> {
  enum { COL_PERMUTATION_IS_NOT_AVAILABLE, YOU_SHOULD_HAVE_ONLY_ONE_SUBINDEX };
};

template <>
struct ei_choose_assert_selection<ColPermutation> {
  enum { ROW_PERMUTATION_IS_NOT_AVAILABLE, YOU_SHOULD_HAVE_ONLY_ONE_SUBINDEX };
};
...
class SubMatrix
 ...
 public:
  ...
  typedef typename ei_choose_container_impl<MatrixType, PermutationType>::type
      MemoryBase;
  typedef typename ei_choose_row_impl<MatrixType, PermutationType, IsSub>::type
      RowBase;
  typedef typename ei_choose_col_impl<MatrixType, PermutationType, IsSub>::type
      ColBase;
  ...
  inline SubMatrix(MatrixType& matrix, const Indices& indices)
      : MemoryBase(matrix), RowBase(matrix, indices), ColBase(matrix, indices) {
    assert(assert_index::YOU_SHOULD_HAVE_ONLY_ONE_SUBINDEX);
  }
  inline SubMatrix(MatrixType& matrix, bool defaultPermutationRows,
...
template <typename MatrixType1, typename MatrixType2>
class StackMatrix : public MatrixBase<StackMatrix<MatrixType1, MatrixType2> > {
 public:
