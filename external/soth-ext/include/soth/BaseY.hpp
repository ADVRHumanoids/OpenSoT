...
class SOTH_EXPORT BaseY {
 protected:
  ...
  typedef HouseholderSequence<const MatrixXd, HCoeffsType_const>
 ...
 public:
  ...
  HouseholderSequenceType_const getHouseholderSequence() const {
    HouseholderSequenceType_const res = HouseholderSequenceType_const(
        householderEssential, householderEssential.diagonal());
  ...
  void multiply(const MatrixBase<DerivedI>& m,
                MatrixBase<DerivedO>& res) const {
    if (isExplicit)
      res = matrixExplicit * m;
    else {
      res = m;
      applyThisOnTheRight(res);
    }
  }
  template <typename DerivedI, typename DerivedO>
  void transposeMultiply(const MatrixBase<DerivedI>& m,
                         MatrixBase<DerivedO>& res) const {
    if (isExplicit)
      res = matrixExplicit.transpose() * m;
    else {
      res = m;
      applyTransposeOnTheRight(res);
    }
  }
};
...
// M := M*Y.
template <typename Derived>
void BaseY::applyThisOnTheLeft(MatrixBase<Derived>& M) const {
  if (isExplicit) {
    MatrixXd tmp = M;
...
// M := M*Y'.
template <typename Derived>
void BaseY::applyTransposeOnTheLeft(MatrixBase<Derived>& M) const {
  if (isExplicit) {
...
// M := Y*M.
template <typename Derived>
void BaseY::applyThisOnTheRight(MatrixBase<Derived>& M) const {
  if (isExplicit) {
    M = matrixExplicit * M;
  } else {
...
// M := Y'*M.
template <typename Derived>
void BaseY::applyTransposeOnTheRight(MatrixBase<Derived>& M) const {
  if (isExplicit) {
