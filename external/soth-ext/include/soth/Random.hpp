...
template <>
inline unsigned int Random::rand() {
  return next();
}
template <>
inline unsigned int Random::randMax() {
  return SOTH_RND_MAX;
}
template <>
...
template <>
inline int Random::randMax() {
  return SOTH_RND_MAX >> 1;
}
template <>
inline double Random::rand() {
  return static_cast<double>(next()) / (SOTH_RND_MAX);
}
template <>
inline double Random::randMax() {
  return 1.;
}
...
class MatrixRnd {
 public:
  ...
  static MatrixBase<Derived>& randomize(
      MatrixBase<Derived>& m,
      typename Derived::Scalar min =
          rnd_traits<typename Derived::Scalar>::default_min(),
      typename Derived::Scalar max =
          rnd_traits<typename Derived::Scalar>::default_max()) {
    for (typename Derived::Index i = 0; i < m.rows(); ++i) {
      for (typename Derived::Index j = 0; j < m.cols(); ++j)
        m(i, j) = static_cast<typename Derived::Scalar>(
                      (max - min) * Random::rand<double>()) +
                  min;
    }
    return m;
  }
