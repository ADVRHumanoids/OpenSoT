 ...
 * the input matrix M and the householder vectors essential parts stored in the
 * column of a different matrix given by the user.
 */
template <typename _MatrixType,
          typename _HouseholderStorageType =
              typename internal::plain_matrix_type<_MatrixType>::type>
class DestructiveColPivQR {
 public:
  ...
  typedef typename HouseholderSequence<
      MatrixQType, HCoeffsType>::ConjugateReturnType HouseholderSequenceType;

  DestructiveColPivQR(MatrixType& matrix,
  ...
  DestructiveColPivQR& compute(MatrixType& matrix,
                               MatrixQType& householderEssentialStorage) {
    m_r = matrix;
    m_q = householderEssentialStorage;
  }
  ...
  RealScalar epsilon() const {
    return m_usePrescribedEpsilon
               ? m_prescribedEpsilon * m_r.diagonalSize()
...
typename MatrixType::RealScalar DestructiveColPivQR<
    MatrixType, HouseholderStrorageType>::absDeterminant() const {
  eigen_assert(m_isInitialized && "DestructiveColPivQR is not initialized.");
...
template <typename MatrixType, typename HouseholderStrorageType>
typename MatrixType::RealScalar DestructiveColPivQR<
    MatrixType, HouseholderStrorageType>::logAbsDeterminant() const {
  eigen_assert(m_isInitialized && "DestructiveColPivQR is not initialized.");
  eigen_assert(m_r.rows() == m_r.cols() &&
               "You can't take the determinant of a non-square matrix!");
...
template <typename MatrixType, typename HouseholderStrorageType>
DestructiveColPivQR<MatrixType, HouseholderStrorageType>&
DestructiveColPivQR<MatrixType, HouseholderStrorageType>::compute() {
  Index rows = m_r.rows();
  Index cols = m_r.cols();
  ...
  if (cols > 1)
    ...
    m_colsIntTranspositions = IntRowVectorType::Zero(
        1);  // TODO: treat this special case here. It just requires to check
             // every output data is correctly initilized
  ...
  // internal::abs2(epsilon()) /rows; The threshold should be decided wrt. to
  ...
  for (Index k = 0; k < size; ++k) {
    ...
    biggest_col_sq_norm = m_r.col(m_colsIntTranspositions[biggest_col_index])
                              .tail(rows - k)
                              .squaredNorm();
    ...
    // biggest column, terminate to avoid generating nan/inf values. Note that
    // here, if we test instead for "biggest == 0", we get a failure every 1000
    // (or so) repetitions of the unit test, with the result of solve() filled
    // with large values of the order of 1/(size*epsilon).
    ...
    if (k != biggest_col_index) {
      std::swap(m_colsIntTranspositions.coeffRef(k),
                m_colsIntTranspositions.coeffRef(biggest_col_index));
      // std::swap(m_colsInvTranspositions.coeffRef(m_colsTranspositions[k]),
      // m_colsInvTranspositions.coeffRef(m_colsTranspositions[biggest_col_index]));
    ...
    // beta);
    VectorBlock<typename MatrixQType::ColXpr> colk =
        m_q.col(k).tail(rows - k - 1);
    m_r.col(m_colsIntTranspositions[k])
        .tail(rows - k)
        .makeHouseholder(colk, m_hCoeffs.coeffRef(k), beta);
    ...
    for (Index l = k + 1; l < cols; ++l)
      m_r.col(m_colsIntTranspositions[l])
          .tail(rows - k)
          .applyHouseholderOnTheLeft(m_q.col(k).tail(rows - k - 1),
    ...
    for (Index l = k + 1; l < cols; ++l)
      m_colSqNorms[l] -= m_r(k, m_colsIntTranspositions[l]) *
                         m_r(k, m_colsIntTranspositions[l]);
  ...
  for (Index k = 0; k < m_nonzero_pivots; ++k)
    m_colsPermutation.applyTranspositionOnTheRight(
        (int)k, (int)m_colsTranspositions.coeff(k));
...
template <typename MatrixType, typename HouseholderStrorageType>
typename DestructiveColPivQR<MatrixType,
                             HouseholderStrorageType>::HouseholderSequenceType
DestructiveColPivQR<MatrixType, HouseholderStrorageType>::householderQ() const {
  eigen_assert(m_isInitialized && "DestructiveColPivQR is not initialized.");
  HouseholderSequenceType hh(
      m_q, m_hCoeffs.conjugate());  //, false, m_nonzero_pivots, 0);
