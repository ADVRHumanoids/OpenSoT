...
class Givens {
 ...
 public:
  SOTH_EXPORT void makeGivens(double a, double b, Index i, Index j,
                              double* z = 0);
  ...
  void applyThisOnTheLeft(
      SubMatrix<MatrixType, PermutationType, IsSub>& M) const;
  // M := M*G'.
  template <typename D>
  void applyTransposeOnTheLeft(MatrixBase<D>& M) const;
  template <typename MatrixType, int PermutationType, bool IsSub>
  void applyTransposeOnTheLeft(
      SubMatrix<MatrixType, PermutationType, IsSub>& M) const;
  // M := G*M.
  template <typename D>
  ...
  template <typename MatrixType, int PermutationType, bool IsSub>
  void applyThisOnTheLeftPartiel(
      SubMatrix<MatrixType, PermutationType, IsSub>& M) const;
  ...
  template <typename MatrixType, int PermutationType, bool IsSub>
  void applyTransposeOnTheRight(
      SubMatrix<MatrixType, PermutationType, IsSub>& M, const Index size) const;
...
template <typename MatrixType, int PermutationType, bool IsSub>
void operator>>(const GivensSequence& G,
                SubMatrix<MatrixType, PermutationType, IsSub>& M) {
  G.applyThisOnTheRight(M);
}
template <typename MatrixType, int PermutationType, bool IsSub>
...
static void applyRotation(MatrixBase<Derived>& x, MatrixBase<Derived>& y,
                          Rot j);

/* --- Construction single ------------------------------------------------ */
template <typename VectorBase>
inline Givens::Givens(const VectorBase& v, Index i, Index j) : i(i), j(j) {
...
// M := M*G.
template <typename Derived>
void Givens::applyThisOnTheLeft(MatrixBase<Derived>& M) const {
  // original solution is not working because "innerstride" is not define in
  // general M.applyOnTheRight(i, j, G);
...
// M := M*G.
template <typename Derived>
void Givens::applyThisOnTheLeftPartiel(MatrixBase<Derived>& M) const {
  if ((Index(i) < M.cols()) && (Index(j) < M.cols()))
    M.applyOnTheRight(i, j, G);
  else if (Index(i) < M.cols()) {
    assert(M.col(i).norm() < 1e-6);
  } else if (Index(j) < M.cols()) {
    assert(M.col(j).norm() < 1e-6);
  }
}

// M := M*G'.
template <typename Derived>
void Givens::applyTransposeOnTheLeft(MatrixBase<Derived>& M) const {
  // original solution is not working because "innerstride" is not define in
  // general M.applyOnTheRight(i, j, G.adjoint());
  typename Derived::ColXpr x(M.col(i));
  typename Derived::ColXpr y(M.col(j));
  applyRotation(x, y, G);
}
...
// M := G*M.
template <typename Derived>
void Givens::applyThisOnTheRight(MatrixBase<Derived>& M) const {
  // original solution is not working because "innerstride" is not define in
  // general M.applyOnTheLeft(i, j, G);
...
// M := G'*M.
template <typename Derived>
void Givens::applyTransposeOnTheRight(MatrixBase<Derived>& M) const {
  // original solution is not working because "innerstride" is not define in
  // general M.applyOnTheLeft(i, j, G.adjoint());
...
static void applyRotation(MatrixBase<Derived>& x, MatrixBase<Derived>& y,
                          Rot j) {
  // TODO:
  //   - Depending on the value of has_direct_access<Derived>::ret, use the
  //   jacobi solution
...
// M := M*G.
template <typename MatrixType, int PermutationType, bool IsSub>
void Givens::applyThisOnTheLeft(
    SubMatrix<MatrixType, PermutationType, IsSub>& M) const {
...
// M := M*G'.
template <typename MatrixType, int PermutationType, bool IsSub>
void Givens::applyTransposeOnTheLeft(
    SubMatrix<MatrixType, PermutationType, IsSub>& M) const {
...
// M := G'*M.
template <typename MatrixType, int PermutationType, bool IsSub>
void Givens::applyTransposeOnTheRight(
    SubMatrix<MatrixType, PermutationType, IsSub>& M) const {
...
// M := G*M.
template <typename MatrixType, int PermutationType, bool IsSub>
void Givens::applyThisOnTheRight(
    SubMatrix<MatrixType, PermutationType, IsSub>& M) const {
...
// M := M*G.
template <typename MatrixType, int PermutationType, bool IsSub>
void Givens::applyThisOnTheLeftPartiel(
    SubMatrix<MatrixType, PermutationType, IsSub>& M) const {
...
// M := M*G.
template <typename Derived>
void GivensSequence::applyThisOnTheLeft(MatrixBase<Derived>& M) const {
...
// M := M*G.
template <typename Derived>
void GivensSequence::applyThisOnTheLeftReduced(MatrixBase<Derived>& M) const {
  for (size_t i = 0; i < G.size(); ++i) G[i].applyThisOnTheLeftPartiel(M);
}

// M := M*G'.
template <typename Derived>
void GivensSequence::applyTransposeOnTheLeft(MatrixBase<Derived>& M) const {
  typedef typename MatrixBase<Derived>::Index Index;
  for (Index i = G.size() - 1; i >= 0; --i) G[i].applyTransposeOnTheLeft(M);
}

// M := G*M.
template <typename Derived>
void GivensSequence::applyThisOnTheRight(MatrixBase<Derived>& M) const {
  typedef typename MatrixBase<Derived>::Index Index;
...
// M := G'*M.
template <typename Derived>
void GivensSequence::applyTransposeOnTheRight(MatrixBase<Derived>& M) const {
  for (size_t i = 0; i < G.size(); ++i) G[i].applyTransposeOnTheRight(M);
}

/* --- Multiplication display -------------------------------------------- */
template <>
inline MATLAB::MATLAB(MatrixXd::Index size, const GivensSequence& m1) {
  if (size == 0)
    initMatrixColNull(0);
  else {
    MatrixXd Yex(size, size);
    Yex.setIdentity();
    m1.applyThisOnTheLeft(Yex);
    initMatrix(Yex);
  }
}
