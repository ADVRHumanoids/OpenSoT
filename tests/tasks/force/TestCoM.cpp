...
TEST_F(testForceCoM, testForceCoP) {
    ...
    //SET UP FORCE OPTIMIZATION
    yarp::sig::Matrix ft_T_sole = coman_robot.idynutils.iDyn3_model.getPosition(
        coman_robot.idynutils.iDyn3_model.getLinkIndex(ft_in_contact[0]),
        coman_robot.idynutils.iDyn3_model.getLinkIndex("l_sole"));

    yarp::sig::Vector wrench_d(12,0.0);
    OpenSoT::tasks::force::CoP::Ptr force_copL_task(
                new OpenSoT::tasks::force::CoP(wrench_d, coman_robot.idynutils,
                                               std::fabs(ft_T_sole(2,3)), ft_in_contact[0]));
    OpenSoT::tasks::force::CoP::Ptr force_copR_task(
                new OpenSoT::tasks::force::CoP(wrench_d, coman_robot.idynutils,
                                               std::fabs(ft_T_sole(2,3)), ft_in_contact[1]));
    OpenSoT::tasks::force::CoM::Ptr force_com_task(
                new OpenSoT::tasks::force::CoM(wrench_d, coman_robot.idynutils));
    force_com_task->setLambda(400.0,40.0);
    std::list<tasks::velocity::Cartesian::TaskPtr> force_aggregated_list;
    force_aggregated_list.push_back(force_copL_task);
    force_aggregated_list.push_back(force_copR_task);
    //force_aggregated_list.push_back(force_com_task);
    Task<Matrix, Vector>::TaskPtr taskForceAggregated =
            tasks::Aggregated::TaskPtr(
       new tasks::Aggregated(force_aggregated_list,wrench_d.size()));


    OpenSoT::solvers::QPOases_sot::Stack stack_of_tasks_force;
    stack_of_tasks_force.push_back(taskForceAggregated);
    OpenSoT::solvers::QPOases_sot::Ptr sot_force(
                new OpenSoT::solvers::QPOases_sot(stack_of_tasks_force,2E5));
    sot_force->solve(wrench_d);
...
//            }
        ...
        taskForceAggregated->update(wrench_d);
