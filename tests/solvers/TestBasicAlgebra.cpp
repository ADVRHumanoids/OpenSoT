...
TEST_F(testBasicAlgebra, checkProductOptimization)
{
    ...
    Eigen::VectorXd t_naive = Eigen::VectorXd::Zero(iters);
    Eigen::VectorXd t_opt = Eigen::VectorXd::Zero(iters);

    // First we test diagonal weights
    for(uint i = 0; i < iters; ++i)
    {
        Eigen::MatrixXd A = Eigen::MatrixXd::Random(rows, cols);
        Eigen::MatrixXd At = A.transpose();

        Eigen::MatrixXd W = 13.0 * Eigen::MatrixXd::Identity(rows, rows);
        Eigen::MatrixXd WA = W*A;

        auto tic = std::chrono::high_resolution_clock::now();
        Eigen::MatrixXd H_naive = At*WA; //naive implementation
        auto toc = std::chrono::high_resolution_clock::now();
        t_naive[i] = std::chrono::duration_cast<std::chrono::nanoseconds>(toc-tic).count()*1e6;

        Eigen::MatrixXd H(cols, cols); //optimized implementation
        tic = std::chrono::high_resolution_clock::now();
        H.triangularView<Eigen::Upper>() = At*WA;
        H = H.selfadjointView<Eigen::Upper>();
        toc = std::chrono::high_resolution_clock::now();
        t_opt[i] = std::chrono::duration_cast<std::chrono::nanoseconds>(toc-tic).count()*1e6;

        for(uint j = 0; j < rows; ++j)
        {
            for(uint k = 0; k < rows; ++k)
            {
                EXPECT_NEAR(H_naive(j, k), H(j, k), 1e-12) << "Mismatch at (" << j << ", " << k << ")";
            }
        }
    }

    double mean_naive = t_naive.sum() / iters;
    double mean_opt = t_opt.sum() / iters;
    std::cout << "Naive implementation mean time: " << mean_naive << " us" << std::endl;
    std::cout << "Optimized implementation mean time: " << mean_opt << " us" << std::endl;

    // Now we test PD symmetric gains
    for(uint i = 0; i < iters; ++i)
    {
        Eigen::MatrixXd A = Eigen::MatrixXd::Random(rows, cols);
        Eigen::MatrixXd At = A.transpose();

        Eigen::MatrixXd W = Eigen::MatrixXd::Random(rows, rows);
        W = W * W.transpose();
        Eigen::MatrixXd WA = W*A;
    ...
    mean_naive = t_naive.sum() / iters;
    mean_opt = t_opt.sum() / iters;
    std::cout << "Naive implementation mean time: " << mean_naive << " us" << std::endl;
    std::cout << "Optimized implementation mean time: " << mean_opt << " us" << std::endl;
}
