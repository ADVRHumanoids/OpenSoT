...
Eigen::VectorXd Postural::getReference() const
{
    return _x_desired;
}

void Postural::getReference(Eigen::VectorXd &x_desired,
...
void Postural::setLambda(double lambda)
{
    if(lambda >= 0.0){
        this->_lambda = lambda;
        this->update_b();
...
Eigen::VectorXd Postural::getActualPositions()
{
    return _x;
}
